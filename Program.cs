    ...
    switch (menuSelection)
    {
        ...
        case "2":
            ...
            while (dogCharacteristics.Length == 0)
            {
                ...
                if (readResult != null)
                {
                    dogCharacteristics = readResult.ToLower().Trim().Split(',', StringSplitOptions.TrimEntries);
                    Console.WriteLine();
                }
            }
            ...
            for (int i = 0; i < maxPets; i++)
            {
                ...
                if (ourAnimals[i, 1].Contains("dog"))
                {
                    ...
                    string[] matchedTerms = new string[0];
                    ...
                    foreach (string characteristic in dogCharacteristics)
                    {
                        ...
                        if (dogDescription.Contains(characteristic))
                        {
                            ...
                            Array.Resize<string>(ref matchedTerms, matchedTerms.Length + 1);
                            matchedTerms[matchedTerms.Length - 1] = characteristic;
                            noMatchesDog = false;
                        }
                    }

                    if(matchedTerms.Length > 0){
                        Console.WriteLine($"\nOur dog {ourAnimals[i, 3]} is a match for your search!");
                        Console.WriteLine($"They matched on:");
                        foreach (string term in matchedTerms)
                        {
